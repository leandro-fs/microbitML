...
Esta actividad inspiró la creación del framework en 2024. Su nombre, `microbitML`, responde a que se pensó originalmente para explorar conceptos de Machine Learning en el aula. Complementa las funcionalidades de [Micro:bit CreateAI](https://microbit.org/ai/), pudiendo servir como introducción a tema "modelos de ML", para luego entrenar modelos con CreateIA.
...
Todos los micro:bits ejecutan el mismo programa (`.hex`). Lo único que los diferencia es el rol y el grupo configurados en cada uno.
...
`microbitML` guarda automáticamente la configuración y la recuerda aunque se apague el dispositivo. Evitar el grupo 0: cualquier nodo que se reinicie sin config previa vuelve a ese grupo.
...
Dar tiempo a que cada grupo interactúe con los dispositivos y debata entre sí hasta llegar a una hipótesis.

### Fase 2 — Introducción al vocabulario de ML

Una vez que los grupos infirieron las respuestas, el docente puede introducir los siguientes conceptos sobre lo que los estudiantes ya experimentaron:
...
- Z suma las entradas ponderadas y aplica una **función de activación** (*activation function*). Su salida puede ser una **clasificación** final, o bien —en una **arquitectura** más compleja— alimentar hacia adelante (*feed forward*) otra **capa** (*layer*) de perceptrones en un **MLP**. Z puede también aplicar un **sesgo** (*bias*), también parte del *modelo*.
- Los *pesos* de A y B son fijos en esta demo, pero la parte "Learning" del **ML Supervisado** (*Supervised ML*) consiste en ajustarlos mediante miles de ciclos de exposición a **muestras etiquetadas** (*tagged samples*) que forman un **dataset**. Cada ciclo mejora el modelo aplicando pequeños cambios en los *pesos y sesgo*. Eso se llama **entrenamiento** (*training*), y continúa hasta que alguna métrica resulte satisfactoria para los **científicos de datos** (*data scientists*) que diseñaron el **pipeline de ML**.
- Como parte del *pipeline*, el *modelo* se **valida** contra un *dataset* completamente nuevo. Si las métricas se mantienen, el modelo se considera listo para producción; de lo contrario, se necesita más *entrenamiento*.
- **Conclusión para el aula:** al consumir dispositivos con ML incorporado, siempre tener en cuenta que los errores de clasificación van a ocurrir, especialmente si el *modelo* subyacente no fue entrenado para la aplicación en cuestión.
...
Basado en el artículo del Prof. Fujio Yamamoto: [building a Microbit network emulating a MLP](https://sparse-dense.blogspot.com/2018/06/microbittwo-layer-perceptronxor.html). 山本さん、本当にありがとうございました！
